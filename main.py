...
from core.query import exp_to_level, interactive_query, get_level_range, level_to_min_exp
...
def cmd_query(args):
    ...
    if args.interactive:
        ...
        print(f"经验值: {exp}")
        print(f"等级: {level}")
        print(f"当前等级范围: {min_exp} - {max_exp if max_exp != float('inf') else '∞'}")
        ...
        if 1 <= level <= 50:
            ...
            print(f"等级范围: {min_exp} - {max_exp if max_exp != float('inf') else '∞'}")
        else:
...
"""
    ...
    query_group.add_argument('--interactive', action='store_true', help='交互式查询模式')
    ...
    chart_parser.add_argument('--max-exp', type=int, default=50000, help='最大经验值 (默认: 50000)')
    ...
    dist_parser.add_argument('--file', help='数据文件路径 (默认: datas/group_statistics.csv)')
